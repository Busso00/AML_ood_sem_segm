...
def train(args, model, enc=False):
    ...
    if args.model == "erfnetIsoMax":
      criterion = IsoMaxPlusLossSecondPart(entropic_scale=10.0).cuda()
    ...
    for epoch in range(start_epoch, args.num_epochs+1):
        ...
        if args.model != "bisenetv1":
          ...
          for step, (images, labels) in enumerate(loader):
              ...
              if args.model == "erfnetIsoMax":
                  """
                  targets_one_hot = targets.squeeze(1)
                  targets_one_hot = F.one_hot(targets_one_hot, num_classes=20)  # Shape: [8, 512, 1024, 20]
                  targets_one_hot = targets_one_hot.permute(0, 3, 1, 2)  # Shape: [8, 20, 512, 1024]
                  targets_one_hot = targets_one_hot.permute(1,0,2,3)
                  s = targets.size() 
                  targets = targets_one_hot.reshape(s[0], -1) # 8*512*1024, 20 -> normalization along dim 1 by default
                  """
