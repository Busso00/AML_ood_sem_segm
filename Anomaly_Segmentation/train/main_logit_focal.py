...
from eval.bestT import FocalLoss
...
def train(args, model, enc=False):
    ...
    if args.model == "erfnet":
      entropy_loss = CrossEntropyLoss2d(weight)

      freq=torch.Tensor([
         0.3292293643951416,
         0.04726318836212158,
         0.19158866214752196,
         0.006409337997436523,
         0.00717833137512207,
         0.012936335563659668,
         0.0017297258377075196,
         0.005827383041381836,
         0.15133179950714112,
         0.007272659301757812,
         0.02928550148010254,
         0.011339406013488769,
         0.001879256248474121,
         0.05699092769622803,
         0.0026325883865356446,
         0.003399106025695801,
         0.0009842872619628905,
         0.0006951541900634766,
         0.006199695587158203,
         1.0
      ])
      ...
      logit_loss = LogitNormLoss(device=args.cuda, t = args.temp, weight = weight)
      focal_loss = FocalLoss(gamma=2) #mean reduction by default

      def combined_loss(outputs, targets):
        loss_ce = entropy_loss(outputs, targets)
        loss_logitnorm = logit_loss(outputs, targets)
        loss_focal = focal_loss(outputs, targets)
        return loss_ce + loss_logitnorm + loss_focal
...
if __name__ == '__main__':
    ...
    parser.add_argument('--temp', type=float, default=1.0, help='Temperature scaling for LogitNormLoss')
